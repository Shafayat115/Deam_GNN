...
def window_indices(pos, length, win):
    ...
    if win >= 1 and pos + 1 < length:
        indices.append(pos + 1)
    if win >= 2 and pos - 1 >= 0:
        indices.insert(0, pos - 1)
    if win >= 3 and pos + 2 < length:
        indices.append(pos + 2)
    if win >= 4 and pos - 2 >= 0:
        indices.insert(0, pos - 2)
    if win >= 5 and pos + 3 < length:
        indices.append(pos + 3)
    if win >= 6 and pos - 3 >= 0:
    ...
    for g in df2[group_col].unique():
        ...
        if (this_fold == high_f and g_targets == {1}) or (this_fold == low_f and g_targets == {0}):
            for h in df2[group_col].unique():
                ...
                if (
                    (this_fold == high_f and that_fold == low_f and h_targets == {0})
                    or (this_fold == low_f and that_fold == high_f and g_targets == {0})
                ):
                    df_sim = df2.copy()
...
df["is_test"] = 0  # kept for compatibility with downstream scripts
...
for cutoff in rate_cutoffs:
    ...
    for win in window_sizes:
        ...
        train_windowseq = full_df[group_col].tolist()
        train_target = full_df[f"Rate_{cutoffstr}"].astype(int).tolist()
        ...
        while swaps < max_swaps:
            ...
            new_assignments = refine_twofold_swap(
                df_train, group_col, "Target", assignments_fold.copy(), global_pos_pct
            )
        ...
        full_df[col_name] = assignments_fold.values
...
print(f"\nWrote full split dataset to {output_csv}")
...
# Example of train/val split for CV for window w and cutoff c:
...
for cutoff in rate_cutoffs:
    ...
    for fold in range(n_folds):
        ...
        for i, (stat, stats_label) in enumerate(zip(stats, stats_labels)):
            ...
            bars = plt.bar(
                x + offset,
                fold_data[stat].reindex(group_sizes_plot, fill_value=0),
    ...
    plt.legend()
    plt.xlim(group_sizes_plot[0] - 0.5, group_sizes_plot[-1] + 0.2)
    plt.tight_layout()
    ...
    for fold in range(n_folds):
        ...
        bars_neg = ax.bar(
            x + offset, all_neg, width,
            label="Negatives" if fold == 0 else "",
            color=bar_colors["neg"], alpha=0.5, zorder=5
        )
        bars_pos = ax.bar(
            x + offset, all_pos, width, bottom=all_neg,
            label="Positives" if fold == 0 else "",
            color=bar_colors["pos"], alpha=0.8, zorder=5
        )
        bars_mix = ax.bar(
            x + offset, all_mix, width,
            bottom=(np.array(all_neg) + np.array(all_pos)),
            label="Mixed Clusters" if fold == 0 else "",
            color=bar_colors["mix"], alpha=0.7, zorder=5
        )
